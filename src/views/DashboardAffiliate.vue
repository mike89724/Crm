...
<template>
  <div id="dashboard-swaps">
    <div class="flex my-10">
      <generic-card style="margin-right: 5%;" :width="componentWidth(uiComponents[0].length)" :value="uiComponents[0][index].value" :name="uiComponents[0][index].name" :title="uiComponents[0][index].title" :key="index" v-for="(componentItem, index) in uiComponents[0].slice(0,2)">
      </generic-card>
    </div>
    ...
    <div class="flex justify-between my-10">
      <generic-card :width="componentWidth(uiComponents[1].length)" :value="uiComponents[1][index].value" :name="uiComponents[1][index].name" :title="uiComponents[1][index].title" :key="index" v-for="(componentItem, index) in uiComponents[1].slice(0,1)">
      </generic-card>
    </div>
    <div class="flex justify-between my-10">
...
export default {
  ...
  methods: {
    ...
    async getHomeData() {
      var response;
      try {
        response = await PageService.getPageDetails("affiliates");
